...
bool COWLCStake::InitFromTxIn(const CTxIn& txin)
{
...
bool COWLCStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool COWLCStake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool COWLCStake::GetTxOutFrom(CTxOut& out) const
{
...
bool COWLCStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount COWLCStake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool COWLCStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
CDataStream COWLCStake::GetUniqueness() const
{
    //The unique identifier for a OWLC stake is the outpoint
...
CBlockIndex* COWLCStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool COWLCStake::ContextCheck(int nHeight, uint32_t nTime)
{
