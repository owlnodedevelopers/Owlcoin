...
#include "qt/owlcoin/forms/ui_snackbar.h"
#include "qt/owlcoin/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(OwlcoinGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &OwlcoinGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
