...
#include "qt/owlcoin/forms/ui_send.h"
#include "qt/owlcoin/addnewcontactdialog.h"
#include "qt/owlcoin/qtutils.h"
#include "qt/owlcoin/sendchangeaddressdialog.h"
#include "qt/owlcoin/optionbutton.h"
#include "qt/owlcoin/sendconfirmdialog.h"
#include "qt/owlcoin/myaddressrow.h"
#include "qt/owlcoin/guitransactionsutils.h"
...
SendWidget::SendWidget(OwlcoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-owlc");
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        CTxDestination owlcAdd = DecodeDestination(address.toStdString());
        if (walletModel->isMine(owlcAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(owlcAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
