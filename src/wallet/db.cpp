...
CDB::CDB(const std::string& strFilename, const char* pszMode, bool fFlushOnCloseIn) : pdb(NULL), activeTxn(NULL)
{
...

    {
        ...
        strFile = strFilename;
        ++biowlc.mapFileUseCount[strFile];
        pdb = biowlc.mapDb[strFile];
        if (pdb == NULL) {
            pdb = new Db(biowlc.dbenv, 0);

            bool fMockDb = biowlc.IsMock();
            if (fMockDb) {
...
void CDB::Flush()
{
    ...
    biowlc.dbenv->txn_checkpoint(nMinutes ? GetArg("-dblogsize", 100) * 1024 : 0, nMinutes, 0);
}
...
bool CDB::Rewrite(const std::string& strFile, const char* pszSkip)
{
    while (true) {
        {
            LOCK(biowlc.cs_db);
            if (!biowlc.mapFileUseCount.count(strFile) || biowlc.mapFileUseCount[strFile] == 0) {
                // Flush log data to the dat file
                biowlc.CloseDb(strFile);
                biowlc.CheckpointLSN(strFile);
                biowlc.mapFileUseCount.erase(strFile);
                ...
                { // surround usage of db with extra {}
                    ...
                    Db* pdbCopy = new Db(biowlc.dbenv, 0);
                ...
                if (fSuccess) {
                    Db dbA(biowlc.dbenv, 0);
                    if (dbA.remove(strFile.c_str(), NULL, 0))
                        fSuccess = false;
                    Db dbB(biowlc.dbenv, 0);
