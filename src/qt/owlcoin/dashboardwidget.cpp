...
#include "qt/owlcoin/dashboardwidget.h"
#include "qt/owlcoin/forms/ui_dashboardwidget.h"
#include "qt/owlcoin/sendconfirmdialog.h"
#include "qt/owlcoin/txrow.h"
...
DashboardWidget::DashboardWidget(OwlcoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareOWLC, "square-chart-owlc");
    setCssProperty(ui->labelOWLC, "text-chart-owlc");
    ...
    setCssProperty(ui->labelAmountOWLC, "text-stake-owlc-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &OwlcoinGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &OwlcoinGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &OwlcoinGUI::openFAQ);
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            owlc = (pair.first != 0) ? pair.first / 100000000 : 0;
            chartData->totalOWLC += pair.first;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesOWLC.append(owlc);

        int max = std::max(owlc, owlc);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->append(chartData->valuesOWLC);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalOWLC > 0) {
        setCssProperty(ui->labelAmountOWLC, "text-stake-owlc");
    } else {
        setCssProperty(ui->labelAmountOWLC, "text-stake-owlc-disable");
    }
    forceUpdateStyle({ui->labelAmountOWLC});
    ui->labelAmountOWLC->setText(GUIUtil::formatBalance(chartData->totalOWLC, nDisplayUnit));
