...
*Setup instructions for a gitian build of Owlcoin using a VM or physical system.*

Gitian is the deterministic build process that is used to build the Owlcoin
...
to the Owlcoin GitHub release page.
...
- [Building Owlcoin](#building-owlcoin-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/owlcoin-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/owlcoin-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/owlcoin-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/owlcoin-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/owlcoin-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/owlcoin-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/owlcoin-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@owlnode.online) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/owlcoin-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/owlcoin-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/owlcoin-osx-*-build.assert
```
