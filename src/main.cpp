...
#if defined(NDEBUG)
#error "Owlcoin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uowlc) are considered zero fee (for relaying, mining and transaction creation)
...
bool RecalculateOWLCSupply(int nHeightStart)
{
	AssertLockHeld(cs_main);
	...
	uiInterface.ShowProgress(_("Recalculating OWLC supply..."), 0);
	while (true) {
		if (pindex->nHeight % 1000 == 0) {
			LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
			int percent = std::max(1, std::min(99, (int)((double)((pindex->nHeight - nHeightStart) * 100) / (chainHeight - nHeightStart))));
			uiInterface.ShowProgress(_("Recalculating OWLC supply..."), percent);
		}
...
{
    util::ThreadRename("owlcoin-scriptch");
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> owlcInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
                owlcInputs.push_back(stakeIn);
        }
        const bool hasOWLCInputs = !owlcInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasOWLCInputs) {
                    ...
                    for (const CTxIn& owlcIn : owlcInputs)
                        if(owlcIn.prevout == in.prevout)
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction& t : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        ...
                        if (hasOWLCInputs) {
                            for (const CTxIn& stakeIn : owlcInputs)
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        // Owlcoin: We use certain sporks during IBD, so check to see if they are
