...
/**
 * Fees smaller than this (in uowlc) are considered zero fee (for transaction creation)
    ...
    const std::vector<unsigned char>& vchCryptedSecret)
{
    ...
    {
        LOCK(cs_wallet);
        if (pwalleowlcEncryption)
            return pwalleowlcEncryption->WriteCryptedKey(vchPubKey,
                vchCryptedSecret,
...
bool CWallet::SetMinVersion(enum WalletFeature nVersion, CWalletDB* pwalleowlcIn, bool fExplicit)
{
    ...
    if (fFileBacked) {
        CWalletDB* pwalleowlc = pwalleowlcIn ? pwalleowlcIn : new CWalletDB(strWalletFile);
        if (nWalletVersion > 40000)
            pwalleowlc->WriteMinVersion(nWalletVersion);
        if (!pwalleowlcIn)
            delete pwalleowlc;
    }
...
bool CWallet::EncryptWallet(const SecureString& strWalletPassphrase)
{
...

    {
        ...
        if (fFileBacked) {
            assert(!pwalleowlcEncryption);
            pwalleowlcEncryption = new CWalletDB(strWalletFile);
            if (!pwalleowlcEncryption->TxnBegin()) {
                delete pwalleowlcEncryption;
                pwalleowlcEncryption = NULL;
                return false;
            }
            pwalleowlcEncryption->WriteMasterKey(nMasterKeyMaxID, kMasterKey);
        }

        if (!EncryptKeys(vMasterKey)) {
            if (fFileBacked) {
                pwalleowlcEncryption->TxnAbort();
                delete pwalleowlcEncryption;
            }
        ...
        SetMinVersion(FEATURE_WALLETCRYPT, pwalleowlcEncryption, true);

        if (fFileBacked) {
            if (!pwalleowlcEncryption->TxnCommit()) {
                delete pwalleowlcEncryption;
...
int64_t CWallet::IncOrderPosNext(CWalletDB* pwalleowlc)
{
    AssertLockHeld(cs_wallet); // nOrderPosNext
    int64_t nRet = nOrderPosNext++;
    if (pwalleowlc) {
        pwalleowlc->WriteOrderPosNext(nOrderPosNext);
    } else {
...
bool CWallet::AddToWallet(const CWalletTx& wtxIn, bool fFromLoadWallet, CWalletDB* pwalleowlc)
{
    ...
    if (fFromLoadWallet) {
        ...
        if (fInsertedNew) {
            ...
            wtx.nOrderPos = IncOrderPosNext(pwalleowlc);
            wtxOrdered.insert(std::make_pair(wtx.nOrderPos, TxPair(&wtx, (CAccountingEntry*)0)));
            wtx.UpdateTimeSmart();
            AddToSpends(hash);
        }
...
bool CWalletTx::WriteToDisk(CWalletDB *pwalleowlc)
{
    if (pwalleowlc)
        return pwalleowlc->WriteTx(GetHash(), *this);
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX_OLD)); // Old 50 OWLC collateral
...
bool CWallet::GetBudgetFinalizationCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    std::string strFail = "";
    std::vector<std::pair<CScript, CAmount> > vecSend;
    vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX)); // New 5 OWLC collateral
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true) {
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-owlcoin-address
                    CScript scriptChange;
...
{
    {
        ...
        LogPrintf("CommitTransaction:\n%s", wtxNew.ToString());
        {
            ...
            CWalletDB* pwalleowlc = fFileBacked ? new CWalletDB(strWalletFile, "r+") : NULL;
            ...
            AddToWallet(wtxNew, false, pwalleowlc);
...
bool CWallet::AddAccountingEntry(const CAccountingEntry& acentry, CWalletDB & pwalleowlc)
{
    if (!pwalleowlc.WriteAccountingEntry_Backend(acentry))
        return false;
