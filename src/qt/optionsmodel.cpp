...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::OWLC);
    ...
    if (settings.contains("nAnonymizeOwlcoinAmount") || reset)
        SoftSetArg("-anonymizeowlcoinamount", settings.value("nAnonymizeOwlcoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in owlcoin.cpp during shotdown to also remove the window positions
