...
#include "qt/owlcoin/splash.h"
#include "qt/owlcoin/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("owlcoin-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in owlcoin.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in owlcoin.qrc)
...
/** Class encapsulating Owlcoin startup and shutdown.
...
/** Main Owlcoin application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (OwlcoinGUI)
...
{
    window = new OwlcoinGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &OwlcoinGUI::detectShutdown);
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &OwlcoinGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(OwlcoinGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(OwlcoinGUI::DEFAULT_WALLET);
        ...
        // Owlcoin: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &OwlcoinGUI::handlePaymentRequest);
        connect(window, &OwlcoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. Owlcoin can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(owlcoin_locale);
    Q_INIT_RESOURCE(owlcoin);
    ...
    /// 6. Determine availability of data directory and parse owlcoin.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("Owlcoin"),
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(0, QObject::tr("Owlcoin"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // owlcoin: links repeatedly have their payment requests routed to this process:
...
#if defined(Q_OS_WIN)
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("Owlcoin didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
