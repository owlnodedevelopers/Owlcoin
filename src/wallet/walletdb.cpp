...
void ThreadFlushWalletDB(const std::string& strFile)
{
    ...
    util::ThreadRename("owlcoin-wallet");
    ...
    while (true) {
        ...
        if (nLastFlushed != nWalletDBUpdated && GetTime() - nLastWalletUpdate >= 2) {
            TRY_LOCK(biowlc.cs_db, lockDb);
            if (lockDb) {
                // Don't do this if any databases are in use
                int nRefCount = 0;
                std::map<std::string, int>::iterator mi = biowlc.mapFileUseCount.begin();
                while (mi != biowlc.mapFileUseCount.end()) {
                    nRefCount += (*mi).second;
                    mi++;
                }
                ...
                if (nRefCount == 0) {
                    ...
                    std::map<std::string, int>::iterator mi = biowlc.mapFileUseCount.find(strFile);
                    if (mi != biowlc.mapFileUseCount.end()) {
                        LogPrint(BCLog::DB, "Flushing wallet.dat\n");
                        nLastFlushed = nWalletDBUpdated;
                        int64_t nStart = GetTimeMillis();

                        // Flush wallet.dat so it's self contained
                        biowlc.CloseDb(strFile);
                        biowlc.CheckpointLSN(strFile);

                        biowlc.mapFileUseCount.erase(mi++);
...
bool BackupWallet(const CWallet& wallet, const fs::path& strDest, bool fEnableCustom)
{
    ...
    while (true) {
        {
            LOCK(biowlc.cs_db);
            if (!biowlc.mapFileUseCount.count(wallet.strWalletFile) || biowlc.mapFileUseCount[wallet.strWalletFile] == 0) {
                // Flush log data to the dat file
                biowlc.CloseDb(wallet.strWalletFile);
                biowlc.CheckpointLSN(wallet.strWalletFile);
                biowlc.mapFileUseCount.erase(wallet.strWalletFile);
